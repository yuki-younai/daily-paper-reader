...
title_zh: 人工智能与对称性：物理数据中结构的表示、编码与发现
authors: Veronica Sanz
date: 2026-02-02
pdf: "https://arxiv.org/pdf/2602.02351v1"
tags: ["query:sr"]
score: 8.0
evidence: 识别并编码物理数据中的对称性约束
tldr: 本综述探讨了人工智能与物理对称性之间的相互作用，重点研究如何利用机器学习（特别是变分自编码器）从高维物理数据中识别、编码和诊断对称性约束。不同于预设对称性的架构，本文侧重于数据驱动的潜在表示学习，分析对称性和守恒定律如何降低数据集的内在维度，并探讨在无显式归纳偏置下推断对称结构的理论与实践局限。
selection_source: fresh_fetch
motivation: 旨在探索如何利用AI的表示学习能力，从高维物理数据中自动发现和提取由对称性决定的低维结构。
method: 重点分析了基于数据驱动的潜在表示学习方法，特别是通过变分自编码器在重建与压缩平衡中实现潜在空间的自组织。
result: 通过几何系统和粒子物理的案例研究，展示了对称性如何导致物理数据集内在维度的降低及其在生成模型中的体现。
conclusion: 总结了在缺乏显式归纳偏置的情况下，从数据中推断对称性结构的潜力及其在理论和实践上的局限性。
---

## 摘要
对称性在物理学中起着核心作用，它组织动力学、约束相互作用，并决定物理自由度的有效数量。与此同时，现代人工智能方法通过表示学习展示了从高维数据中提取低维结构的卓越能力。本综述探讨了这两个视角之间的相互作用，重点关注利用机器学习技术在多大程度上可以识别、编码或诊断由对称性诱导的约束。我们不侧重于通过构建来强制执行已知对称性的架构，而是专注于数据驱动方法和潜表示学习，并特别关注变分自编码器。我们讨论了对称性和守恒定律如何降低物理数据集的内在维度，以及这种维度的降低如何通过在旨在平衡重构与压缩的生成模型中潜空间的自组织来体现。我们回顾了最近的研究结果，包括来自简单几何系统和粒子物理过程的案例研究，并分析了在没有显式归纳偏置的情况下推断对称结构的理论和实践局限性。
