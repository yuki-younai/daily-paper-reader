...
tags: ["query:sr"]
score: 8.0
evidence: 从受损测量数据中发现控制方程
tldr: 针对物理信息神经网络（PINN）在处理复杂测量噪声和异常值时性能下降的问题，本文提出了噪声自适应物理信息神经网络（naPINN）。该方法无需预验噪声分布，通过嵌入基于能量的模型学习残差分布，并利用可训练的可靠性门控自适应过滤高能量异常数据，结合拒绝成本正则化防止误删。实验证明，naPINN在多种非高斯噪声和高异常值率的偏微分方程基准测试中表现优异，能准确重建物理动力学。
selection_source: fresh_fetch
motivation: 传统的PINN在面对复杂的非高斯噪声和大量异常值时，由于缺乏对噪声分布的先验知识，往往难以准确恢复物理规律。
method: 提出naPINN框架，利用能量模型学习预测残差的潜在分布，并通过可训练的可靠性门控和拒绝成本正则化实现对异常数据的自适应过滤。
result: 在多个受非高斯噪声和不同比例异常值干扰的偏微分方程基准测试中，naPINN显著优于现有的鲁棒PINN基线方法。
conclusion: naPINN能够有效隔离异常值并在严重数据损坏的情况下准确重建物理动力学，为解决现实世界中带噪声的逆问题提供了强有力的工具。
---

## 摘要
物理信息神经网络 (PINNs) 是解决反问题和从观测数据中发现控制方程的有效方法。然而，在复杂的测量噪声和严重的离群值影响下，其性能会显著下降。为解决这一问题，我们提出了噪声自适应物理信息神经网络 (naPINN)，它能够在无需噪声分布先验知识的情况下，从受损测量中稳健地恢复物理学解。naPINN 在训练循环中嵌入了一个基于能量的模型，以学习预测残差的潜在分布。利用学习到的能量景观，一个可训练的可靠性门控自适应地过滤掉表现出高能量的数据点，同时拒绝成本正则化防止了丢弃有效数据的平凡解。我们在受非高斯噪声和不同比例离群值干扰的各种基准偏微分方程上证明了 naPINN 的有效性。结果表明，naPINN 显著优于现有的稳健 PINN 基准模型，能够成功隔离离群值，并在严重的数据损坏情况下准确重建动力学过程。
