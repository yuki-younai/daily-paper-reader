...
tldr: 当前多任务监督微调（SFT）通常对所有子数据集采用统一的计算预算，导致学习动态不一致，部分任务过拟合而另一些欠拟合。本文提出mSFT，一种感知过拟合的迭代搜索算法。它通过识别并剔除最早过拟合的数据集，并回滚至最优检查点继续训练，从而优化数据混合。实验证明mSFT在多个基准和模型上优于基线，能在提升性能的同时降低计算开销。
selection_source: fresh_fetch
motivation: 解决多任务SFT中由于子数据集学习速度不一导致的异构过拟合问题。
method: 提出一种迭代算法，动态识别并剔除已过拟合的任务，并回退到该任务的最优状态继续训练。
result: 在10个基准测试和6个基础模型上，mSFT一致优于4种基线方法，且对超参数不敏感。
conclusion: mSFT为多任务SFT提供了一种实用的过拟合感知方案，能有效最大化模型在多样化数据混合下的潜力。
---
