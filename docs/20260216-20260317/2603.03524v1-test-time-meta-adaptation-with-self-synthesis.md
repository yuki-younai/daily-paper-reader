...
tags: ["query:rlaif"]
score: 8.0
evidence: 用于LLM自我适应和更新后性能优化的元学习框架
tldr: 本研究针对大语言模型在推理阶段适应多样化任务的需求，提出了MASS元学习框架。该框架允许模型在测试时通过生成特定问题的合成训练数据并进行针对性自我更新来提升性能。通过双层优化机制，内循环在合成数据上进行微调，外循环利用元梯度优化数据生成质量，从而实现高效的测试时自适应。在数学推理任务上的实验证明，MASS能生成个性化的课程，显著提升推理效率和准确性。
selection_source: fresh_fetch
motivation: 旨在解决大语言模型在面对多样化领域和任务时，如何在推理阶段实现高效的自我适应与性能提升。
method: 提出MASS框架，利用双层优化和元梯度技术，使模型能够生成针对性合成数据并据此进行自我更新。
result: 在数学推理实验中，MASS成功实现了高效且数据节省的测试时自适应，并能为每个实例生成有效的学习课程。
conclusion: MASS证明了通过元学习优化合成数据生成，可以显著增强大语言模型在推理时的自我进化能力。
---

## 摘要
作为强大的通用推理器，大语言模型（LLMs）会面临各种领域和任务，其中在测试时进行自适应和自我提升的能力具有重要价值。我们提出了 MASS，这是一个元学习框架，它使 LLMs 能够通过生成针对特定问题的合成训练数据，并在推理时执行针对下游性能优化的定向自我更新来实现自适应。我们通过双层优化端到端地训练这种行为：内循环在自生成的样本上进行自适应，而外循环则元学习数据归因信号并奖励更新后的任务性能。合成数据通过可扩展的元梯度进行优化，通过内循环更新反向传播下游损失，从而奖励有用的生成内容。在数学推理上的实验表明，MASS 能够学习合成针对每个实例的课程，从而实现有效且数据高效的测试时自适应。
