...
tags: ["query:rlaif"]
score: 9.0
evidence: 针对大模型智能体的检索增强策略优化
tldr: 针对大语言模型智能体在强化学习中探索受限的问题，本文提出检索增强策略优化（RAPO）框架。该框架通过引入外部检索机制，在训练中将离策（off-policy）步骤级轨迹融入智能体的推理过程，打破了仅依赖自生成输出的局限。RAPO包含混合策略展开和检索感知策略优化两个阶段，显著提升了智能体的探索能力和训练效率。
selection_source: fresh_fetch
motivation: 现有的智能体强化学习方法主要依赖同策探索，限制了发现新推理视角的能力，且缺乏对细粒度步骤级动态的利用。
method: 提出RAPO框架，通过混合策略展开让智能体在检索到的步骤级轨迹上推理，并利用检索感知策略优化机制进行梯度校准。
result: "在三个推理任务的14个数据集上，RAPO实现了5.0%的平均性能提升，且训练速度提高了1.2倍。"
conclusion: RAPO通过显式引入检索机制扩展了智能体的探索空间，为提升大语言模型智能体的复杂任务处理能力提供了高效的新方案。
---

## 摘要
智能体强化学习 (Agentic RL) 在基于大语言模型 (LLM) 的智能体中展现出巨大潜力。这些工作使 LLM 智能体能够通过多步、集成工具的推理来处理复杂任务。然而，现有 Agentic RL 方法的一个固有局限是它们依赖纯同策略 (on-policy) 范式进行探索，这限制了探索仅限于智能体自身生成的输出，阻碍了发现新的推理视角以进一步提升性能。虽然近期的一些工作引入了辅助异策略 (off-policy) 信号来增强探索，但它们通常利用完整的异策略轨迹进行轨迹级策略估计，忽视了智能体展开过程中细粒度、步级探索动态的必要性。在本文中，我们重新审视了 Agentic RL 中的探索问题，并提出了检索增强策略优化 (RAPO)，这是一种在训练期间引入检索以显式扩展探索的新型强化学习框架。为了实现这一目标，我们将 Agentic RL 训练过程分解为两个阶段：(i) 混合策略智能体展开，以及 (ii) 检索感知策略优化。具体而言，我们提出了一种混合策略智能体展开策略，允许智能体在检索到的异策略步级轨迹上持续推理。它动态地扩展了智能体的推理感受野，使其能够在外部行为的引导下进行更广泛的探索。随后，我们引入了检索感知策略优化机制，通过检索奖励和重要性塑造来校准策略梯度估计，从而稳定训练并优先考虑检索启发的探索。大量实验表明，RAPO 在三个智能体推理任务的 14 个数据集上实现了 +5.0% 的平均增益，同时训练效率提升了 1.2 倍。
