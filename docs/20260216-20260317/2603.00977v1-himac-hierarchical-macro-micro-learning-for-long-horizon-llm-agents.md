...
method: 提出一种层次化强化学习框架，将决策分解为宏观蓝图生成与微观目标执行，并采用无评论家的层次化策略优化。
result: 在ALFWorld、WebShop和Sokoban等多个环境下的实验表明，HiMAC在性能和样本效率上均显著优于现有基准。
conclusion: 结构化的层次化设计是提升大模型智能体处理长程复杂任务能力的关键因素。
---
