...
tags: ["query:rlaif"]
score: 8.0
evidence: 大模型驱动的自适应深度强化学习框架用于智能体任务
...
method: 提出一种结合大语言模型与符号规划的闭环框架，通过语义标注自动创建选项并实现实时约束监控。
result: "在Office World和Montezuma's Revenge实验中，该方法在数据效率、约束合规性和跨任务迁移方面表现优异。"
conclusion: 利用LLM的通用知识进行语义驱动的选项发现，能有效增强强化学习系统的适应性、安全性和可解释性。
---
