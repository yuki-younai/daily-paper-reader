...
title_zh: SCALAR：通过 LLM 引导的符号规划与深度强化学习落地实现技能的学习与组合
...
tags: ["query:rlaif"]
score: 9.0
evidence: 将大模型规划与强化学习结合以实现底层控制对齐
...
motivation: 现有的LLM智能体在底层控制上表现不佳，且缺乏纠正初始规范错误的反馈机制。
method: 提出SCALAR框架，通过LLM规划技能并利用RL训练底层策略，结合双向反馈机制动态优化技能定义。
result: "在Craftax基准测试中，钻石收集率达到88.2%（提升1.9倍），并成功进入了此前方法无法到达的矮人矿山。"
conclusion: 结合LLM的高层推理能力与RL的底层执行反馈，是构建复杂环境下鲁棒智能体的有效途径。
---

## 摘要
基于语言模型（LM）的智能体在给定高层动作 API 时表现出色，但在将语言落地为底层控制方面却面临困难。先前的研究让 LLM 为强化学习（RL）生成技能或奖励函数，但这些单次（one-shot）方法缺乏纠正规范错误的反馈。我们提出了 SCALAR，这是一个通过学习到的技能库将 LLM 规划与 RL 耦合的双向框架。LLM 提出带有前置条件和效果的技能；RL 为每个技能训练策略，并反馈执行结果以迭代优化规范，从而提高对初始错误的鲁棒性。关键轨迹分析（Pivotal Trajectory Analysis）通过分析 RL 轨迹来修正 LLM 先验；边界检查点（Frontier Checkpointing）可选择性地在技能边界保存环境状态，以提高样本效率。在 Craftax 任务中，SCALAR 实现了 88.2% 的钻石收集率，比最佳基准提高了 1.9 倍，并在 9.1% 的情况下到达了侏儒矿山（Gnomish Mines），而先前的方法在这些任务中完全失败。
