...
motivation: 多智能体强化学习系统常依赖稀疏的终端反馈，导致上游决策的贡献被混淆，难以进行准确的决策级信用分配。
method: 引入C3方法，通过冻结上下文、执行固定后续重放以及应用留一法（LOO）基准来隔离并评估单个消息的因果影响。
result: 在五个数学和编程基准测试中，C3在相同预算下均优于现有基准，并展现出更高的信用保真度和更强的智能体间因果依赖。
conclusion: C3通过局部干预有效解决了轨迹级信用扩散问题，为LLM多智能体协作的策略优化提供了更精确的信用分配机制。
---
