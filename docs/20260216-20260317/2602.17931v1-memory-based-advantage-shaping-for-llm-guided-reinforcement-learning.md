...
conclusion: 该研究证明了利用记忆图结合LLM指导能有效平衡学习效率与计算开销，是提升强化学习性能的可靠方案。
---

## 摘要
在奖励稀疏或延迟的环境中，由于学习需要大量的交互，强化学习（RL）面临着高样本复杂度的问题。这一局限性促使研究者利用大语言模型（LLM）进行子目标发现和轨迹引导。虽然 LLM 可以辅助探索，但频繁依赖 LLM 调用引发了关于可扩展性和可靠性的担忧。我们通过构建一个记忆图来应对这些挑战，该图编码了来自 LLM 引导和智能体自身成功尝试（rollouts）的子目标及轨迹。基于该图，我们推导出一个效用函数，用于评估智能体的轨迹与先前成功策略的对齐程度。该效用重塑了优势函数，在不改变奖励的情况下为评论者（critic）提供额外的引导。我们的方法主要依赖离线输入，仅进行偶尔的在线查询，避免了对持续 LLM 监督的依赖。在基准环境中的初步实验表明，与基准 RL 方法相比，该方法提高了样本效率并加快了早期学习速度，且最终回报与需要频繁 LLM 交互的方法相当。
